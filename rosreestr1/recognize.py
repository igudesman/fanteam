...
def recognize(imgData, saveImg = False):
  ...
  processedimage = originalimage.convert('L').point(lambda x : 255 if x > 20 else 0, mode='1').convert('1').convert('RGBA')
    ...
    for i in range(15, 120): # leftmost mask position, rigthmost mask position
      ...
      if resultimage.getextrema() == ((0, 0), (0, 0), (0, 0), (255, 255)):
        if z == 4: # fill 4 with white to exclude false 1 on 4
          ...
          processedimage.paste(Image.alpha_composite(processedimage.crop((i, 0, i + 21, 0 + 50)), maskx), (i, 0))
  ...
  Zlist.sort()
  return None #if less then 5 digits recognized